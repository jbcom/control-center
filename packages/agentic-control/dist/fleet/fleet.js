...
import { getEnvForRepo, getEnvForPRReview, extractOrg, } from "../core/tokens.js";
...
export class Fleet {
    ...
    async inboundLoop(config, agentIds, processedIds, interval) {
        while (true) {
            try {
    ...
    async processCoordinationComment(config, agentIds, comment) {
        ...
        if (body.includes("✅ DONE:")) {
            ...
            if (match) {
                ...
                agentIds.delete(agentId);
                this.postPRComment(config.repo, config.coordinationPr, `✅ Acknowledged completion from ${agentId.slice(0, 12)}. Summary: ${summary}`);
            }
        }
            ...
            if (match) {
                ...
                this.postPRComment(config.repo, config.coordinationPr, `⚠️ Agent ${agentId.slice(0, 12)} blocked: ${issue}\n\nManual intervention may be required.`);
            }
        }
    }
    /**
    ...
    fetchPRComments(repo, prNumber) {
        try {
            ...
            const output = execSync(`gh api repos/${repo}/issues/${prNumber}/comments --jq '.[] | {id: .id, body: .body, author: .user.login, createdAt: .created_at, updatedAt: .updated_at}'`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"], env });
            return output
                .trim()
                .split("\n")
                .filter(Boolean)
    ...
    postPRComment(repo, prNumber, body) {
        try {
            ...
            execSync(`gh pr comment ${prNumber} --repo ${repo} --body-file -`, { input: body, stdio: ["pipe", "pipe", "pipe"], env });
        }
        catch (err) {
