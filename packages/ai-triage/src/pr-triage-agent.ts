...
const FeedbackItemSchema = z.object({
  ...
  category: z.enum(["bug", "security", "performance", "style", "documentation", "suggestion", "question"]),
...
const PRAnalysisSchema = z.object({
  ...
  blockers: z.array(z.object({
    type: z.enum(["ci_failure", "unaddressed_feedback", "merge_conflict", "missing_approval", "other"]),
...
## Blockers to Fix
${autoResolvable.map((b, i) => `${i + 1}. [${b.type}] ${b.description}
   Suggested fix: ${b.suggestedFix || 'Analyze and fix'}`).join('\n\n')}

## Unaddressed Feedback
${unaddressedFeedback.map((f, i) => `${i + 1}. [${f.severity}] ${f.source}: ${f.content}
