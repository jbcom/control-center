...
`;
  ...
  fetchPRComments(repo: string, prNumber: number): PRComment[] {
    try {
      const token = process.env.GITHUB_JBCOM_TOKEN || process.env.GH_TOKEN;
      const envPrefix = token ? `GH_TOKEN="${token}" ` : "";
      const output = execSync(
        `${envPrefix}gh api repos/${repo}/issues/${prNumber}/comments --jq '.[] | {id: .id, body: .body, author: .user.login, createdAt: .created_at}'`,
  ...
  postPRComment(repo: string, prNumber: number, body: string): void {
    try {
      const token = process.env.GITHUB_JBCOM_TOKEN || process.env.GH_TOKEN;
      const envPrefix = token ? `GH_TOKEN="${token}" ` : "";
      execSync(`${envPrefix}gh pr comment ${prNumber} --repo ${repo} --body-file -`, { 
        input: body, 
  ...
  /**
   * Watch fleet for status changes, reporting periodically
   */
  async watch(options?: {
    pollInterval?: number;
    onStatusChange?: (agent: Agent, oldStatus: AgentStatus, newStatus: AgentStatus) => void;
    ...
    console.log(`🔭 Watching fleet (poll every ${pollInterval / 1000}s)...`);

    while (iteration < maxIterations) {
      const result = await this.list();
      if (result.success && result.data) {
        for (const agent of result.data) {
          const oldStatus = statusMap.get(agent.id);
          if (oldStatus && oldStatus !== agent.status) {
            console.log(`📊 Agent ${agent.id.slice(0, 12)}... status: ${oldStatus} → ${agent.status}`);
            options?.onStatusChange?.(agent, oldStatus, agent.status);
          }
          statusMap.set(agent.id, agent.status);
        }

        const running = result.data.filter(a => a.status === "RUNNING").length;
        const finished = result.data.filter(a => ["COMPLETED", "FINISHED"].includes(a.status)).length;
        console.log(`📈 Fleet: ${running} running, ${finished} finished, ${result.data.length} total`);
      }
  ...
  /**
   * Monitor specific agents until they complete
   */
  async monitorAgents(agentIds: string[], options?: {
    ...
    console.log(`👁️ Monitoring ${agentIds.length} agents...`);

    while (remaining.size > 0 && (Date.now() - startTime) < timeout) {
      for (const agentId of Array.from(remaining)) {
        const result = await this.status(agentId);
        if (result.success && result.data) {
          const agent = result.data;
          if (agent.status === "COMPLETED" || agent.status === "FINISHED") {
            console.log(`✅ Agent ${agentId.slice(0, 12)}... completed`);
            remaining.delete(agentId);
            results.set(agentId, agent);
            options?.onComplete?.(agent);
          } else if (agent.status === "FAILED" || agent.status === "CANCELLED") {
            console.log(`❌ Agent ${agentId.slice(0, 12)}... ${agent.status.toLowerCase()}`);
            remaining.delete(agentId);
            results.set(agentId, agent);
            options?.onFailed?.(agent);
          }
        }
      }

      if (remaining.size > 0) {
        console.log(`⏳ Waiting for ${remaining.size} agents...`);
        await new Promise(resolve => setTimeout(resolve, pollInterval));
      }
    }

    if (remaining.size > 0) {
      console.log(`⏰ Timeout reached. ${remaining.size} agents still running.`);
    }
   ...
   * - OUTBOUND: Check agent status, send followups
   * - INBOUND: Poll PR comments, dispatch actions
   */
  async coordinate(config: CoordinationConfig): Promise<void> {
    ...
    console.log(`🎯 Starting coordinator for PR #${config.coordinationPr}`);
    console.log(`   Repo: ${config.repo}`);
    console.log(`   Agents: ${agentIds.size > 0 ? Array.from(agentIds).join(", ") : "(discovering)"}`);
    console.log(`   Outbound interval: ${outboundInterval / 1000}s`);
    console.log(`   Inbound interval: ${inboundInterval / 1000}s`);
    ...
    // OUTBOUND loop - check agents and send updates
    const outboundLoop = async () => {
      while (running) {
        try {
          // Discover running agents if none specified
          if (agentIds.size === 0) {
          ...
          // Check status of tracked agents
          ...
          for (const agentId of agentIds) {
            ...
            if (result.success && result.data) {
              ...
              if (agent.status === "COMPLETED" || agent.status === "FINISHED") {
                ...
                this.postPRComment(config.repo, config.coordinationPr,
                  `@cursor ✅ DONE: \`${agentId.slice(0, 12)}\` finished with status ${agent.status}`
                );
              } else if (agent.status === "FAILED") {
                completed.push(agentId);
                this.postPRComment(config.repo, config.coordinationPr,
                  `@cursor ❌ FAILED: \`${agentId.slice(0, 12)}\` - ${agent.summary ?? "Unknown error"}`
                );
              }
            }
          }
          ...
          console.log(`📤 Outbound: tracking ${agentIds.size} agents`);
        } catch (err) {
          console.error("Outbound error:", err);
        }

        await new Promise(resolve => setTimeout(resolve, outboundInterval));
      }
    };

    // INBOUND loop - process PR comments
      ...
      while (running) {
        try {
          ...
          for (const comment of comments) {
            ...
            const cursorMatch = comment.body.match(/@cursor\s+(✅|⚠️|📊|🔄|DONE|BLOCKED|STATUS|HANDOFF):\s*(.+)/i);
            if (cursorMatch) {
              const [, type, content] = cursorMatch;
              console.log(`📥 Inbound: ${type} from ${comment.author}: ${content.slice(0, 50)}...`);

              // Extract agent ID if present
              const agentIdMatch = content.match(/\b(bc-[a-f0-9-]+)/);
              if (agentIdMatch) {
                const msgAgentId = agentIdMatch[1];
                
                // Handle different message types
                if (type === "✅" || type.toUpperCase() === "DONE") {
                  agentIds.delete(msgAgentId);
                  console.log(`   Agent ${msgAgentId.slice(0, 12)} marked complete`);
                } else if (type === "⚠️" || type.toUpperCase() === "BLOCKED") {
                  console.log(`   ⚠️ Agent ${msgAgentId.slice(0, 12)} blocked: ${content}`);
                }
              }
            }
          }
        ...
        await new Promise(resolve => setTimeout(resolve, inboundInterval));
      }
    };

    // Run both loops concurrently
    await Promise.all([outboundLoop(), inboundLoop()]);
  }
}
