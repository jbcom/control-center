...
from terraform_modules import doppler_config, settings
...
CLIENT_INPUTS = {
    ...
    "get_vault_client": {
        ...
        "ACTIONS_ID_TOKEN_REQUEST_URL": {"required": "false", "sensitive": "false"},
        "ACTIONS_ID_TOKEN_REQUEST_TOKEN": {"required": "false", "sensitive": "true"},
        "VAULT_URL": {"required": "false", "sensitive": "false"},
        "VAULT_NAMESPACE": {"required": "false", "sensitive": "false"},
        "VAULT_TOKEN": {"required": "false", "sensitive": "true"},
        "VAULT_ROLE_ID": {"required": "false", "sensitive": "true"},
        "VAULT_SECRET_ID": {"required": "false", "sensitive": "true"},
        "VAULT_APPROLE_PATH": {"required": "false", "sensitive": "false"},
    },
    "get_zoom_client": {
    ...
    for k, v in d.items():
        ...
        if not is_nothing(v):
            if isinstance(v, Mapping):
                ...
                if k == "description":
                    filtered[k] = truncate(v, max_length=settings.MAX_DESCRIPTION_LENGTH)
                    continue
...
# ----------------------
# GitHub Actions Environment Variable Utilities
# ----------------------


def format_github_env_value(key: str, value: Any, prefix: str = "") -> str:
    """
    Format a key-value pair for GITHUB_ENV file using heredoc syntax.

    This handles multi-line values (certificates, private keys, etc.) safely
    by using GitHub Actions' heredoc format:
    ...
    Args:
        ...
        value: The value (can be any type, will be converted to string/JSON)
        prefix: Optional prefix to add to the key (e.g., "TF_VAR_")

    Returns:
        Properly formatted string for appending to GITHUB_ENV
    """
    ...
    # Use heredoc format - delimiter includes key name to ensure uniqueness
    delimiter = f"__GH_EOF_{full_key}__"

    # Ensure delimiter doesn't appear in value (extremely unlikely but safe)
    counter = 0
    while delimiter in str_value:
        counter += 1
        delimiter = f"__GH_EOF_{full_key}_{counter}__"

    return f"{full_key}<<{delimiter}\n{str_value}\n{delimiter}"


def format_github_env_entries(
    data: Mapping[str, Any],
    prefix: str = "",
    key_transform: Optional[str] = None,
) -> str:
    """
    Format a dictionary of key-value pairs for GITHUB_ENV file.

    Args:
        data: Dictionary of environment variables to export
        prefix: Optional prefix for all keys (e.g., "TF_VAR_")
        key_transform: Optional transformation: "upper", "lower", "snake_case"

    Returns:
        Multi-line string ready to append to GITHUB_ENV
    """
    lines = []
    ...
    for key, value in data.items():
        ...
        elif key_transform == "snake_case":
            transformed_key = inflection.underscore(key)

        lines.append(format_github_env_value(transformed_key, value, prefix))

    return "\n".join(lines)


def write_to_github_env(
    data: Mapping[str, Any],
    prefix: str = "",
    key_transform: Optional[str] = None,
    ...
    This is the primary function for exporting variables in GitHub Actions.
    It automatically detects the GITHUB_ENV file path from the environment.

    Args:
        data: Dictionary of environment variables to export
        prefix: Optional prefix for all keys (e.g., "TF_VAR_")
        key_transform: Optional transformation: "upper", "lower", "snake_case"
        env_file: Override GITHUB_ENV file path (auto-detected if not provided)

    Returns:
        True if successfully written, False if not in GitHub Actions environment
    """
    if env_file is None:
    ...
    formatted = format_github_env_entries(data, prefix, key_transform)
...
def write_to_github_output(
    ...
    Args:
        data: Dictionary of outputs to export
        output_file: Override GITHUB_OUTPUT file path (auto-detected if not provided)
    ...
    formatted = format_github_env_entries(data)
...
class Utils:
    def __init__(
        ...
        **kwargs,
        ...
        # Store raw stdin data separately for methods that need direct access
        ...
        if from_stdin and not strtobool(os.getenv("OVERRIDE_STDIN", False)):
            ...
            if not is_nothing(inputs_from_stdin):
                try:
                    self._stdin_data = json.loads(inputs_from_stdin)
                    inputs = self._stdin_data | inputs
        ...
        # Vendor secrets can come from ASM (lambda mode) or Doppler
        # ASM is preferred for Lambda execution - no external token needed
        # TM_VENDORS_SOURCE: 'asm' (ASM only), 'doppler' (Doppler only), 'auto' (try ASM first)
        vendors_source = os.getenv("TM_VENDORS_SOURCE", "auto")
        vendors_loaded = False

        # In Lambda or when explicitly set to 'asm', load from ASM /vendors/*
        if vendors_source in ("asm", "auto"):
            try:
                # Import here to avoid circular dependency
                from terraform_modules.aws_client import load_vendors_from_asm

                vendors = load_vendors_from_asm()
                if vendors:
                    for key, value in vendors.items():
                        self.inputs[key] = value
                    vendors_loaded = True
            except ImportError:
                pass  # aws_client not available, fall back to Doppler
            except Exception:
                pass  # ASM failed, fall back to Doppler

        # Fall back to Doppler if ASM failed or if explicitly set to 'doppler'
        if not vendors_loaded and vendors_source in ("doppler", "auto"):
            ...
            if doppler_token:
                try:
                    ...
                    secrets_response = doppler.secrets.list(
                        project=doppler_config.DOPPLER_PROJECT,
                        config=doppler_config.DOPPLER_CONFIG,
            ...
            if is_url(str(file_path)):
                ...
                if response.ok:
                    ...
                    state_negative_result(
                        f"URL {file_path} could not be read:" f" [{response.status_code}] {response.reason}"
                    )
            else:
