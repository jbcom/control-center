locals {
  ...
  db_subnet_group_name          = local.create_db_subnet_group ? join("", aws_db_subnet_group.this.*.name) : local.db_subnet_group_name
...
resource "aws_db_subnet_group" "this" {
  count = local.create_cluster && local.create_db_subnet_group ? 1 : 0
...
resource "aws_rds_cluster_instance" "this" {
  ...
  performance_insights_kms_key_id       = lookup(each.value, "performance_insights_kms_key_id", local.performance_insights_kms_key_id)
  performance_insights_retention_period = lookup(each.value, "performance_insights_retention_period", local.performance_insights_retention_period)
  ...
  tags = try(merge(local.tags, each.value.tags), local.tags)
}
...
resource "aws_rds_cluster_endpoint" "this" {
  ...
  tags = ry(merge(local.tags, each.value.tags), local.tags)
}
...
resource "aws_security_group" "this" {
  ...
  tags = merge(local.tags, var.security_group_tags, { Name = var.name })
}
...
locals {
  zone_ids_and_names = flatten([
    for zone_id in local.zone_ids : [
      for rds_name, rds_data in module.rds : [
        {
          name             = rds_name
          zone_id          = zone_id
          cluster_endpoint = rds_data.cluster_endpoint
        }
      ]
    ]
  ])
...
resource "aws_route53_record" "rds" {
  for_each = {
    for zone_id_data in local.zone_ids : zone_id_data.name => zone_id_data.zone_id
  }
  ...
  type    = "CNAME"
  ttl     = 60
  records = [local.zone_ids_and_names[count.index]["cluster_endpoint"]]
}