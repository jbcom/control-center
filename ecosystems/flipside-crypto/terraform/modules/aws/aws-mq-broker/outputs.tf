locals {
  broker = var.auto_minor_version_upgrade ? aws_mq_broker.default-no-auto-upgrade : aws_mq_broker.default
}

output "broker_id" {
  value       = join("", local.broker.*.id)
  description = "AmazonMQ broker ID"
}

output "broker_arn" {
  value       = join("", local.broker.*.arn)
