...
def has_key(thedict, keyvalue):
    return thedict.get(keyvalue) is not None
...
def transformLogEvent(log_event):
    ...
    try:
        ...
        if isinstance(json_message, list):
            logger.info(f"[KDFXFORM] JSON object type is <list>.  Assigning to global key name <{_LIST_KEY_NAME_}>")
            processed_log_event[_LIST_KEY_NAME_] = json_message.copy()
        else:
            logger.info(f"[KDFXFORM] JSON object type is {type(json_message)!s}. Using json object direct copy")
