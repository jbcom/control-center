locals {
  ...
  live_infrastructure_data = {
    databases = local.configured_databases

    dynamodb_tables = local.configured_dynamodb_tables

    elasticache_redis = local.configured_elasticache_redis

    opensearch = local.configured_opensearch
...
module "infrastructure_data" {
  ...
  source_maps = [
    ...
    local.configured_database_connection_strings,
  ]
}
