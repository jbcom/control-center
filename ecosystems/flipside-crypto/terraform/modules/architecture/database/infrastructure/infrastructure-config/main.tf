locals {
  infrastructure_raw_cycles = {
    databases = local.databases_config

    dynamodb_tables = local.dynamodb_tables_config

    elasticache_redis = local.elasticache_redis_config

    opensearch = local.opensearch_config
