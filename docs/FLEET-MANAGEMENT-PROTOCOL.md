...
(
  ...
  echo '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"launchAgent","arguments":{
    ...
    "source": {"repository": "https://github.com//terraform-modules", "ref": "main"}
  }}}'
  sleep 5
...
agents['tf-aws-sm'] = spawn_agent(
    repo="/terraform-aws-secretsmanager",
    task="Update jbcom dependencies to latest versions",
    context=f"Control manager: {MY_AGENT_ID}"
)
...
GH_TOKEN="$GITHUB_FSC_TOKEN" gh repo view /terraform-modules
```
...
- FSC expects you to have: GITHUB_FSC_TOKEN for  repos
