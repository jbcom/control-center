...
**Root Cause**: Cursor cloud agents cannot access `/cluster-ops` despite GitHub App being installed (Cursor Bugbot reviews work fine). Requires local agent or manual handling.
...
1. **Cursor cloud agents fail for  org** - Workaround: use local agent
